#Unity3D-ToolChain-StriteR

关于作者:

打野流派 & 造些自己感兴趣的轮子 & 玩家水平过低

知乎 https://www.zhihu.com/people/striter

相关问题/技术咨询请联系 QQ:908666285

*本库需要安装Bakery插件以正确显示场景GI细节.



<技术玩具 / Keywords>


ThePlanet.scene : 对Townscaper部分技术的拙劣实现. https://www.bilibili.com/opus/776541417448144915

![Screenshot_20221127_155848](https://user-images.githubusercontent.com/32827196/204125050-15f36a1d-cbe2-4ae5-aa55-0092e7dfea60.png)

Gargantua.scene 对ShaderToy-Gartantua的拙劣提炼与模仿,贴图采样暂未更新.

<img width="915" height="507" alt="image" src="https://github.com/user-attachments/assets/72de809d-b6dc-4708-b50c-a5d7eff6da59" />

DiningRoom.scene 移轴摄影模拟

<img width="728" height="533" alt="image" src="https://github.com/user-attachments/assets/135246d8-90f9-4ecf-a2df-d77655630545" />

PolyIsland.scene 卡通描边场景以及视差云海
<img width="1014" height="572" alt="image" src="https://github.com/user-attachments/assets/453ee443-350d-4ff6-ba58-6d49d369054b" />

Poolcore.scene 梦核阳光版 体积光 水 散焦 以及 贴花
<img width="1004" height="576" alt="image" src="https://github.com/user-attachments/assets/6cb3ae1b-cd00-430b-9d82-3906affb8688" />

CharacterControl.scene: 基础IK的实现,多足生物以及头动等.
<img width="687" height="827" alt="image" src="https://github.com/user-attachments/assets/28bc55b7-5b6a-4ce1-9566-1145d176b36b" />

TheBook.scene: 3D书页模拟
<img width="926" height="804" alt="image" src="https://github.com/user-attachments/assets/a8769d1d-af40-4ee7-93e6-2491bba55639" />

CornellBox.scene: 类镜中世界DEMO 多个相机+RenderTexture实现
<img width="548" height="426" alt="image" src="https://github.com/user-attachments/assets/b9803a1d-1d6c-4b28-a06a-7f39fc309bf7" />

SphericalHarmonicsL2.scene: 对标源码实现的SHL2球谐
<img width="919" height="515" alt="image" src="https://github.com/user-attachments/assets/394e0285-f51a-424f-8843-07d39790ac0b" />

Dome.scene: 基于Entity接口模式制作的某种游戏 进度10%
<img width="863" height="850" alt="image" src="https://github.com/user-attachments/assets/55085d19-c081-40a3-94d7-3e516cca4578" />

GPUSkinning.scene: GPU蒙皮实现 https://zhuanlan.zhihu.com/p/1953804653406978936
<img width="713" height="670" alt="image" src="https://github.com/user-attachments/assets/40e30fe1-db02-4404-a8de-07968171c73c" />

GPUAnimationSample.scene: GPU动画实现 https://zhuanlan.zhihu.com/p/413815001
<img width="842" height="820" alt="image" src="https://github.com/user-attachments/assets/e5288d5b-c56c-4f7a-b968-e4a08cc67647" />

OffScreenParticle.scene: 离屏粒子实现 https://zhuanlan.zhihu.com/p/1959606027549914096
<img width="819" height="458" alt="image" src="https://github.com/user-attachments/assets/9178f8b7-bbc5-4c04-88b8-6dcd1540b0c4" />

ReflectiveShadowMap.scene: RSM实现
<img width="926" height="523" alt="image" src="https://github.com/user-attachments/assets/fa326068-d856-4989-8c03-ecd9762e7e60" />

PostProcess.scene 自搓的常用后处理与管线实现 (Bloom Focal VHS 等)
<img width="935" height="524" alt="image" src="https://github.com/user-attachments/assets/a20d4aa0-ab96-4055-a76c-b819b7a1c8eb" />

CustomGI.scene 基于预烘焙的Skyight ambience * Lightmap Dir * SH  + 多个IndirectDiffuse Intensity * Light Color 插值构建的半动态GI
<img width="999" height="479" alt="image" src="https://github.com/user-attachments/assets/91792872-b9cb-469f-b3f5-dad7ebd9dbe0" />

AudioAnalysis.scene 为学习(强行自搓)快速傅里叶变换的音频可视化DEMO
<img width="768" height="547" alt="image" src="https://github.com/user-attachments/assets/40afb236-7da1-4242-b192-0cd094fdac87" />

Imposter.scene 参考网络文章独立实现的Impostor

<img width="602" height="319" alt="image" src="https://github.com/user-attachments/assets/67fb433f-d3ef-436a-81b8-3e1d86106d33" />

Pool.scene CPU端程序化Floating

<img width="926" height="519" alt="image" src="https://github.com/user-attachments/assets/3635ebd5-c00f-42c8-9a78-82c53f50e29a" />


Effects.scene 
1. PBR Shader实现(拷贝) 并基于ShaderLab实现代码多态
<img width="1009" height="553" alt="image" src="https://github.com/user-attachments/assets/f7cfdfda-d6c6-41a7-b6ad-5d30e3ac4ca1" />

2. SurfaceEffectFeature 表面特效解耦 https://zhuanlan.zhihu.com/p/1953100797811365054
<img width="937" height="400" alt="image" src="https://github.com/user-attachments/assets/98f84df0-c7bd-4ed5-b062-5bb39738d617" />

3. 预积分SSS与Fake SSS
<img width="994" height="531" alt="image" src="https://github.com/user-attachments/assets/adfa32e4-f445-46e0-9eed-36baff023365" />
<img width="880" height="307" alt="image" src="https://github.com/user-attachments/assets/14f12ba1-be7b-481f-b2b9-c68759b5b267" />

4. SSPR
   
<img width="1026" height="391" alt="image" src="https://github.com/user-attachments/assets/e6d2bdee-5810-413e-9df7-44ef325ef269" />

5. Blinn-Phong TSF Marine (From Natural Selection 2)
   
<img width="715" height="783" alt="image" src="https://github.com/user-attachments/assets/f1411097-1deb-47ef-bff3-3f0ac0502bb6" />

7. GPU Slicing
   
<img width="723" height="505" alt="image" src="https://github.com/user-attachments/assets/6bc27fad-3c5c-4bd4-a468-3f1523f8ccc2" />






<或许会感兴趣的内容 POE>

ColorPalette 参考IQ大佬文章做的拓展以及编辑器,可实现各种归一化数值颜色变化

<img width="320" height="268" alt="image" src="https://github.com/user-attachments/assets/ccc99175-18ea-4140-9d04-c5f7e65ab673" />

Damper 通用编辑器可配置阻尼器,以及对应的函数可视化

<img width="455" height="291" alt="image" src="https://github.com/user-attachments/assets/7bc2fd29-9180-4b35-be7e-71baef93595c" />

EditorExecutableProcess 编辑器可执行Bundle,用于编制离线处理流程 并成功实现CustomGI的预烘焙

<img width="453" height="412" alt="image" src="https://github.com/user-attachments/assets/72969bcd-8224-43a0-8d46-17d57a1c78c6" />

ConditionAttribute 用Attribute的形式制作编辑器,以替换原本繁琐的编辑器代码实现 (同时ShaderGUI也有对应的实现)

<img width="743" height="445" alt="image" src="https://github.com/user-attachments/assets/6cce6fdc-30ad-4627-9fb6-63df41efa5af" />

AssetProcess 用ScriptableObject关联文件夹并进行目录资源导入处理

<img width="456" height="174" alt="image" src="https://github.com/user-attachments/assets/dda1f82f-d837-490c-bf47-b5ee1b263f9f" />

TouchConsole 早年自搓的Runtime Debug工具
<img width="1008" height="575" alt="image" src="https://github.com/user-attachments/assets/57ec2cff-f60b-4e21-ac58-1b0de33ca4ba" />

SpringSimulation.scene 参考书籍实现的2D弹性绳索模拟
<img width="951" height="528" alt="image" src="https://github.com/user-attachments/assets/ba56c1c0-df1e-48c1-a3ce-4b3d63ab452a" />

PathFindingVisualize.scene 寻路模拟, Graph构建与对应的Gizmos
<img width="744" height="527" alt="image" src="https://github.com/user-attachments/assets/cf12ea36-5967-48b0-8a36-82fa954852b0" />

NumericalAnalysis.scene Newton's method与Newton's fractal的CPU实现
<img width="904" height="531" alt="image" src="https://github.com/user-attachments/assets/37f653ed-c98c-4113-b60c-c0a01d6df731" />

Runtime.Geometry 个人长期使用与维护的几何库
GeometryVisualize.scene: 几何库相关的可视化
<img width="996" height="554" alt="image" src="https://github.com/user-attachments/assets/79ff3412-94d0-452d-ace0-f93006bfe9a7" />

MarchingSquare/MarchingCube.scene 程序化生成常用技术的实现

<img width="591" height="756" alt="image" src="https://github.com/user-attachments/assets/54684433-fa07-40c3-abff-102febf7ae8a" />

WaveFunctionCollapse.scene 2D波函数坍塌的实现.

<img width="870" height="522" alt="image" src="https://github.com/user-attachments/assets/fba2fece-80f1-40b9-a41b-05b13bc027ba" />

UGUIExtensions.cs: 基于UGUI实现的各种组件,如Line/Trail Renderer, 镂空等

<img width="932" height="501" alt="image" src="https://github.com/user-attachments/assets/f2648ec0-7a1a-481c-a43b-4d6cf5a818fe" />

HierarchialDataStructureVisualize.scene: 部分几何体结构树的实现与Gizmos

<img width="907" height="505" alt="image" src="https://github.com/user-attachments/assets/5261dd28-6de0-499c-a899-9464d24eece4" />

以及更多内容.
