...
public static class UEngineIntegration
{
    ...
    {
        var totalFields = new Stack<Queue<FieldInfo>>();
        var fieldStack = new Stack<FieldInfo>();

        foreach (var field in _type.GetAllFields(_flags))
            totalFields.Push(new Queue<FieldInfo>(new[] { field }));
        while (totalFields.Count > 0)
        {
            ...
            var shallowField = curFieldStack.Dequeue();
            fieldStack.Push(shallowField);
            var isBaseType = shallowField.FieldType.IsEnum || shallowField.FieldType.IsArray || shallowField.FieldType.IsGenericType || kSerializeBaseType.Contains(shallowField.FieldType);
