...
public static class UNoise
{
    ...
    public static Vector2 ValueUnit2(Vector2 _random) => new Vector2(ValueUnit(new Vector2(_random.y, _random.x)), ValueUnit(_random));
    #endregion
    #region Perlin
    ...
    public  static Vector2 VoronoiUnit(Vector2 _v)
    {
        ...
        for (int i=-1;i<=1;i++)
            for(int j=-1;j<=1;j++)
            {
                ...
                Vector2 cellPos = cell + ValueUnit2(cell);
