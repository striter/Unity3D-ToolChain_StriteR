...
namespace Runtime.Geometry
{
    public partial struct G2Polygon
    {
        ...
        public static G2Polygon ConvexHull(IList<float2> _positions) => ConvexConstructor.QuickHull(_positions);

        public static class ConvexConstructor
        {
            public static G2Polygon GrahamScan(IList<float2> _positions,float _bias = float.Epsilon) 
            {
                var polygonPoints = PoolList<float2>.Empty(nameof(GrahamScan));
        ...
        public static G2Polygon ConcaveHull(IList<float2> _positions,int k = 3,float _bias = float.Epsilon)
        {
            ...
            var currentDirection = kfloat2.right;
            var step = 1;
            ...
            while ((math.distancesq(currentPoint ,firstPoint) > _bias || step == 1) && pointsSet.Count > 0)
            {
                ...
                cPoints.SortDescending(p=>umath.getRadClockwise(p - currentPoint,currentDirection));
                
                var intersect = true;
                for(var i = 0; i < cPoints.Count;i++)
                {
                    var neighbor = cPoints[i];
                    var lastPoint = math.lengthsq(neighbor - firstPoint) < _bias ? 1 : 0;
                    var j = 2;
                    intersect = false;
                    while (!intersect && j < hull.Count - lastPoint)
                    {
                        intersect = new G2Line(hull[step - 1], neighbor).Intersect(new G2Line(hull[step - 1 - j], hull[step - j]));
                        j++;
                    }
                ...
                if (intersect)
                    return ConcaveHull(pointsSet,k+1);
