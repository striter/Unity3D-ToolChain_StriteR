...
#region Class Pool
public class CClassPool
{
    public static Dictionary<Type, Stack<CClassPool>> m_PoolItems { get; private set; } = new Dictionary<Type, Stack<CClassPool>>();
    static void CheckType(Type type)
    {
        if (!m_PoolItems.ContainsKey(type))
            m_PoolItems.Add(type, new Stack<CClassPool>());
    }
    public static T Spawn<T>() where T : CClassPool, new()
    {
        Type type = typeof(T);
        CheckType(type);
        T item;
        if (m_PoolItems[type].Count > 0)
        {
            item = m_PoolItems[type].Pop() as T;
        }
        else
        {
            item = new T();
    ...
    {
        OnPoolRecycle();
        m_PoolItems[this.GetType()].Push(this);
    }
