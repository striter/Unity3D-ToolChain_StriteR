Shader "Hidden/ReflectiveShadowMapSample"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float4 _RandomVectors[512];
            float4 frag (v2f_img input) : SV_Target
            {
                float3 positionWS = TransformNDCToWorld(input.uv);
                float3 normalWS = SAMPLE_TEXTURE2D(_CameraNormalTexture,sampler_CameraNormalTexture,input.uv) * 2 - 1;// WorldSpaceNormalFromDepth(input.uv);
                float4 positionSS = mul(_WorldToShadow,float4(positionWS,1));
                positionSS /= positionSS.w;
                float3 indirect = 0;
                for(int i = 0 ; i < _RandomVectorCount;i++)
                {
                    float4 randomVector = _RandomVectors[i];
                    float3 samplePosition = positionSS + randomVector.xyz;
                    ...
                    indirect += result * randomVector.w;
                }
