Shader "Hidden/ZSBase"
{
	...
	SubShader
	{
		...
		v2f vert (appdata v)
		{
			...
			float3 worldBitangent=cross(worldTangent,o.normal);
			o.worldToTangent=float3x3(normalize(worldTangent),normalize(worldBitangent),o.normal);
			#endif
			TRANSFER_SHADOW(o);
			return o;
		}
		...
		float4 fragBase(v2f i):SV_TARGET
		{
			...
			diffuse*=lightMask.g;
			float3 diffuseCol = _LightColor0.rgb*diffuse+ambient+_Color.rgb;
			float3	startCol=tex2D(_MainTex, i.uv).rgb;
			startCol*=diffuseCol;
			#if _SPECULAR
