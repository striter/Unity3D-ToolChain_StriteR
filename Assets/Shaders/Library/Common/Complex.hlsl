...
float2 cpow( float2 z, float n ) { float r = length( z ); float a = atan2( z.y, z.x ) * n; return pow( r, n )*float2( cos(a), sin(a) ); }
float2 cpow2( float2 z ) { return float2(z.x * z.x - z.y * z.y, 2 * z.x * z.y); }
