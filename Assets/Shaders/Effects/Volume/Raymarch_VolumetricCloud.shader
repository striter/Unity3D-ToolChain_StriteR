    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float4 frag (v2f i) : SV_Target
            {
                ...
                float2 rayDistances =Distance(boxWS,rayWS);

                float distanceBegin = max(0,rayDistances.x);
                float distanceEnd = min(rayDistances.x + rayDistances.y,RawToDistance(SAMPLE_DEPTH_TEXTURE(_CameraDepthTexture,sampler_CameraDepthTexture, ndc).r,ndc));
                ...
                if(distanceTravel>0)
                {
                    ...
                    float scatter=(1-smoothstep(_ScatterRange,1,dot(rayWS.direction,lightDirWS))*_ScatterStrength);
                    float cloudMarchDst = distanceTravel/_RayMarchTimes;
                    float lightMarchDst = distanceTravel/_LightMarchTimes;
                    float dstMarched = distanceBegin;
                    float marchParam=1.0/_RayMarchTimes;
                    for(uint index=0u;index<_RayMarchTimes;index++)
                    {
                        float3 marchPos = rayWS.GetPoint(dstMarched);
                        ...
                        if(cloudDensity<0.01||dstMarched>distanceEnd)
                            break;
                    }
                }
